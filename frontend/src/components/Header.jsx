...
export default function Header({ activeView, activeOutlet, setActiveOutlet, user, alerts, mobileOpen, setMobileOpen }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        {outletScope.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span className="header-outlet-label" style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase' }}>
              Outlet:
            </span>
            <select
              value={activeOutlet}
        ...
        <div style={{ position: 'relative' }}>
          ...
          {showNotifications && (
            <div style={{
              ...
              <h4 style={{ fontSize: '0.88rem', marginBottom: '12px', borderBottom: '1px solid var(--border-color)', paddingBottom: '8px', color: 'var(--text-primary)' }}>
                  ...
                  {alerts.map((alert, idx) => (
                    <div key={idx} style={{
                      ...
                      <div style={{ fontWeight: 600, display: 'flex', justifyContent: 'space-between', color: 'var(--text-primary)' }}>
                        ...
                        <span style={{ color: alert.alert_type === 'urgent' ? 'var(--critical)' : 'var(--warning)', textTransform: 'uppercase', fontSize: '0.68rem' }}>
                          {alert.alert_type}
                        </span>
                      </div>
